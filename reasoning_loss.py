"""Supervised reasoning-path losses and metrics for CAT V2."""
...
class PathPredictionLoss(nn.Module):
    """Cross-entropy over the target reasoning-path sequence."""
...
class ConceptActivationLoss(nn.Module):
    """Multi-label BCE for concepts present in the target path."""
...
class TransitionPriorLoss(nn.Module):
    """Penalize probability mass on graph-invalid transitions."""
...
class CATReasoningLoss(nn.Module):
    """Combined supervised CAT V2 objective."""
...
if __name__ == "__main__":
    ...
    targets = torch.tensor([[2, 3, 1, 0, 0], [4, 5, 6, 1, 0]])
    activation_logits = torch.randn(2, 10, requires_grad=True)
    activation_targets = torch.zeros(2, 10)
    activation_targets[0, [2, 3]] = 1
    activation_targets[1, [4, 5, 6]] = 1
    loss_fn = CATReasoningLoss()
    loss, metrics = loss_fn(
        {"path_logits": logits, "activation_logits": activation_logits},
        targets,
        activation_targets,
    )
    loss.backward()
