"""CLI runner for Very Large Concepts Model (VLCM).

Enables training, path inference, visualizer export, evaluation,
and running the computational memory benchmark.
"""
...
# Ensure the root workspace folder is in python path
...
from vlcm.path_generator import VLCMModel, build_vlcm_model_from_dataset
from vlcm.reasoning_loss import compute_path_metrics
from vlcm.trainer import (
    VLCMTrainer,
    latest_vlcm_checkpoint,
    load_vlcm_checkpoint,
    set_seed,
)
...
def resolve_checkpoint(args) -> Optional[Path]:
    ...
    return latest_vlcm_checkpoint(getattr(args, "checkpoint_dir", "checkpoints/vlcm"))


def train_command(args) -> None:
    set_seed(args.seed)
    dataset, loader = build_dataset_and_loader(args, shuffle=True)
    model = build_vlcm_model_from_dataset(
        dataset,
    ...
    print(
        ...
        f"concepts={dataset.vocab.size()} "
        f"parameters={sum(p.numel() for p in model.parameters()):,}"
    )
    metrics = trainer.fit(epochs=args.epochs, save_every=args.save_every)
    final_checkpoint = latest_vlcm_checkpoint(args.checkpoint_dir)
    print(json.dumps(metrics, indent=2))
    if final_checkpoint:
        print(f"checkpoint={final_checkpoint}")


def load_or_build_system(args) -> VLCMReasoningSystem:
    checkpoint = resolve_checkpoint(args)
    if checkpoint and checkpoint.exists():
        loaded = load_vlcm_checkpoint(checkpoint, device=args.device)
...
def infer_command(args) -> None:
    ...
    print("\nReasoning Path:")
    print(result["explainability_path"])
    print("\nAnswer:")
    print(result["answer"])
...
def evaluate_command(args) -> None:
    ...
    if not checkpoint or not checkpoint.exists():
        raise FileNotFoundError(
            "evaluation requires a checkpoint; run train first or pass --checkpoint"
        )
...
def visualize_command(args) -> None:
    ...
    if checkpoint and checkpoint.exists():
        ...
        if args.question:
            ...
            highlight_path = system.answer(args.question, max_length=args.max_length)[
                "reasoning_path"
            ]
    else:
        dataset = ReasoningDataset(
...
def benchmark_command(args) -> None:
    from vlcm.benchmark import run_compression_benchmarks
    from vlcm.evaluator import run_engineering_domain_test
    
    # Run theoretical & empirical benchmarks
    run_compression_benchmarks()
    
    # Run the Engineering domain concept chain discovery test (force CPU for deterministic propagation)
    run_engineering_domain_test(device="cpu")



def add_shared_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--dataset", default="data/reasoning_dataset.json")
    parser.add_argument("--checkpoint-dir", default="checkpoints/vlcm")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="VLCM Concept Reasoning runner")
    ...
    visualize.add_argument("--output", default="reports/vlcm_graph.png")
